...
LiquidCrystalSPI::LiquidCrystalSPI(uint8_t rs, uint8_t rw, uint8_t enable,
...
LiquidCrystalSPI::LiquidCrystalSPI(uint8_t rs, uint8_t enable,
			     uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3,
			     uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7)
{
...
LiquidCrystalSPI::LiquidCrystalSPI(uint8_t ssPin) //SPI  ##############################
{
  initSPI(ssPin);
...
void LiquidCrystalSPI::init(uint8_t fourbitmode, uint8_t rs, uint8_t rw, uint8_t enable,
			 uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3,
			 uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7)
{
  _rs_pin = rs;
...
void LiquidCrystalSPI::initSPI(uint8_t ssPin) //SPI ##########################################
{
    // initialize SPI:
	_usingSpi = true;
	_latchPin = ssPin;
...
void LiquidCrystalSPI::begin(uint8_t cols, uint8_t lines, uint8_t dotsize) {
  if (lines > 1) {
...
inline void LiquidCrystalSPI::command(uint8_t value) {
  send(value, LOW);
}

inline size_t LiquidCrystalSPI::write(uint8_t value) {
  send(value, HIGH);
...
void LiquidCrystalSPI::pulseEnable(void) {
  if (_usingSpi == false)
  {
    digitalWrite(_enable_pin, LOW);
    delayMicroseconds(1);    
...
void LiquidCrystalSPI::write8bits(uint8_t value) {
  for (int i = 0; i < 8; i++) {
    pinMode(_data_pins[i], OUTPUT);
    digitalWrite(_data_pins[i], (value >> i) & 0x01);
  }
...
void LiquidCrystalSPI::spiSendOut() //SPI #############################
{
